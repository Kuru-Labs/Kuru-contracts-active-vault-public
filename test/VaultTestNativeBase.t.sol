...
contract VaultTest is Test {
    ...
    function setUp() public {
        ...
        //make quote token
        usdc = new USDC(mintAndBurnAuth, mintAndBurnAuth);
        ...
        quoteMultiplier = 10 ** usdc.decimals();
    }
    ...
    function testDepositOnce() public {
        ...
        vault.deposit{value: baseAmount}(baseAmount, quoteAmount);
        vm.stopPrank();
    }

    function testDepositInsufficientNativeAsset() public {
        // create a new usdc
        USDC usdc2 = new USDC(mintAndBurnAuth, mintAndBurnAuth);
        // create mon/usdc market
        orderBook = IOrderBook(
            router.deployProxy(
                IOrderBook.OrderBookType.NATIVE_IN_BASE,
                address(wbtc),
        ...
        ERC1967Proxy proxy = new ERC1967Proxy(address(implementation), "");
        console.log("proxy", address(proxy));
        Vault vaultNew = Vault(payable(address(proxy)));
        vaultNew.initialize(
            sourceOfFunds,
            operator,
            gasCrank,
        ...
        address depositUser = vm.addr(uint256(keccak256("DEPOSIT_USER")));
        vm.prank(mintAndBurnAuth);
        usdc2.mint(depositUser, 100 * quoteMultiplier);
        vm.startPrank(depositUser);
        usdc2.approve(address(vault), 100 * quoteMultiplier);
        vm.expectRevert(Errors.NativeAssetMismatch.selector);
        vaultNew.deposit(100 * baseMultiplier, 100_000 * 10 ** 18);
        vm.stopPrank();
    }
    ...
    function testPostonlyRevertReplaceOrders() public {
        ...
        marginAccount.deposit{value: 1 * baseMultiplier}(_maker, address(wbtc), 1 * baseMultiplier);
    ...
    function testDepositSingleSide() public {
        ...
        vm.startPrank(user);
        vault.depositSingleSide{value: baseMultiplier / 10}(baseMultiplier / 10, baseMultiplier / 20, 0, true);
        vm.stopPrank();
    ...
    function testRevertDepositSingleSideMinOutNotSatisfied() public {
        ...
        vm.startPrank(user);
        vm.expectRevert();
        vault.depositSingleSide{value: baseMultiplier / 10}(baseMultiplier / 10, baseMultiplier / 20, 5000050001, true);
        vm.stopPrank();
    }
    ...
    function testRevertWithdrawMinOutNotSatisfied() public {
        ...
        vault.withdrawSingleSide(_shares, false, 10 ** 17);
        vm.stopPrank();
    }
}
