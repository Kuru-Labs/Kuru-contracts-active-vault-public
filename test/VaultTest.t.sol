...
contract VaultTest is Test {
    ...
    function testDepositOnce() public {
        ...
        vault.deposit(baseAmount, quoteAmount);
        vm.stopPrank();
    }

    function testDepositInsufficientNativeAsset() public {
        // create a new wbtc
        WBTC wbtc2 = new WBTC(mintAndBurnAuth, mintAndBurnAuth);
        // create wbtc/mon market
        orderBook = IOrderBook(
            router.deployProxy(
        ...
        vm.prank(mintAndBurnAuth);
        wbtc2.mint(depositUser, 100 * baseMultiplier);
        vm.startPrank(depositUser);
        wbtc2.approve(address(vault), 100 * baseMultiplier);
    ...
    function testRevertDepositSingleSideMinOutNotSatisfied() public {
        ...
        wbtc.approve(address(vault), baseMultiplier / 10);
        vm.expectRevert();
        vault.depositSingleSide(baseMultiplier / 10, baseMultiplier / 20, 5000050001, true);
        vm.stopPrank();
    }
